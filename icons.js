/* Local icon set — replaces the lucide CDN dependency.
   Geometry follows lucide's conventions: 24x24 viewBox, stroke currentColor,
   2px stroke, round caps and joins, no fill. Rendered once, synchronously,
   with no MutationObserver: createIcons-style DOM rewriting is itself a
   mutation, so observing the document retriggers the renderer forever on a
   page with 200 icons. */
(function () {
  var P = {
  ...
  function render(root) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      var body = P[name];
      if (!body) { el.removeAttribute('data-lucide'); continue; }
  ...
  // Two late passes cover content mounted after first paint. Bounded on purpose.
