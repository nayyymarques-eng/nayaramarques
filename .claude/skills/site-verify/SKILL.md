...
3. **Rendered, desktop.** For every page in `PAGES` (below): navigate, wait about 2.5s for the runtime, run
   `eval(await (await fetch('/harness/check_pages.js')).text())`. Each must return `pass`. `COL-08 <ratio> "<text>"` is text below WCAG AA contrast on its ground (CLAUDE.md COL-08).
